...
export default {
  ...
  computed: {
    isGoalUnlocked() {
      // Check if all costs are paid and the goal is marked as unlocked
      return (
    ...
    // Compute the background style dynamically based on the goal index
    backgroundStyle() {
      return {
  ...
  methods: {
    ...
    payCost(token, value) {
      ...
      if (storedAmount >= value) {
        ...
        // Check if goal is unlocked after paying
        if (this.goal.costs.every((cost) => this.getPaidCost(cost.token) >= cost.value)) {
          ...
          this.$emit('goal-unlocked'); // Emit an event to notify the parent component
        }

        this.$forceUpdate(); // Force a UI update
      }
    },
    getTokenIcon(token) {
      try {
    ...
    checkGoalUnlock() {
      // This method checks whether the goal is unlocked and triggers reactivity if necessary
  ...
  mounted() {
    // Continuously check if the goal is unlocked (to update UI live)
    this.intervalId = setInterval(() => {
      this.checkGoalUnlock();
